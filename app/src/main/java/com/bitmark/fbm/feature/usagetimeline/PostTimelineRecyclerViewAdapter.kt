...
class PostTimelineRecyclerViewAdapter(period: Period) : UsageTimelineRecyclerViewAdapter(period) {
    ...
    fun add(postModelView: List<PostModelView>) {
        if (postModelView.isEmpty()) return
        val pos = this.data.size
        val sortedData = postModelView.sortedByDescending { it.timestamp }
        val groupedData = sortedData.groupBy { groupKeyBy(period, it.timestamp) }
        val postData = mutableListOf<PostItem>()
        groupedData.forEach { (key, value) ->
            val headerPos = postData.size
            value.forEach { v ->
                val type = when (v.type) {
                    PostType.UPDATE -> UPDATE
                    PostType.MEDIA -> when {
                        v.isMultiMediaPost() -> MULTI_MEDIA
                        v.mediaType == MediaType.PHOTO -> PHOTO
                        else -> VIDEO
                    }
                ...
                postData.add(PostItem(type, headerPos, key, v))
            }
        }
        this.data.addAll(postData)
        notifyItemRangeInserted(pos, postData.size)
    }
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        val data = this.data[position] as PostItem
        when (data.type) {
            UPDATE -> (holder as UpdateVH).bind(data)
            PHOTO -> (holder as PhotoVH).bind(data)
            VIDEO -> (holder as VideoVH).bind(data)
            STORY -> (holder as StoryVH).bind(data)
            LINK -> (holder as LinkVH).bind(data)
            MULTI_MEDIA -> (holder as MultiMediaVH).bind(data)
        }
    }
    ...
    open class VH(
        ...
        protected fun getInfo(item: PostItem): Spanned {
            val post = item.post!!
            val context = itemView.context!!
            val date = DateTimeUtil.millisToString(
                post.timestamp, if (period == Period.DECADE) {
            ...
            if (post.hasLocation()) {
                info.append(" ").append(context.getString(R.string.at_format).format(post.location))
            }
    ...
    class MultiMediaVH(view: View, period: Period, listener: OnItemClickListener?) :
        ...
        fun bind(item: PostItem) {
            ...
            with(itemView) {
                ...
                tvCaptionMedia.text = post.content
                ...
                for (mediaData in post.mediaData!!) {
    ...
    class VideoVH(view: View, period: Period, listener: OnItemClickListener?) :
        ...
        fun bind(postItem: PostItem) {
            this.item = postItem.post!!
            with(itemView) {
                tvInfoVideo.text = getInfo(postItem)
    ...
    class PhotoVH(view: View, period: Period, listener: OnItemClickListener?) :
        ...
        fun bind(item: PostItem) {
            ...
            with(itemView) {
                ...
                tvCaptionPhoto.text = post.content
                ivPhoto.load(post.url!!, post.uri)
            }
        }
    }
    ...
    class StoryVH(view: View, period: Period, listener: OnItemClickListener?) :
        ...
        fun bind(item: PostItem) {
            ...
            with(itemView) {
                ...
                tvCaptionStory.text = post.title
                ivStory.load(post.url ?: "", post.uri)
            }
        }
    }
    ...
    class LinkVH(view: View, period: Period, listener: OnItemClickListener?) :
        ...
        fun bind(item: PostItem) {
            ...
            with(itemView) {
                ...
                if (post.url != null) {
                    val spannableString = SpannableString(post.url)
